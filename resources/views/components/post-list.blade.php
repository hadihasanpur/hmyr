            ...
            <a href="{{ route('posts.show', $post->slug) }}"
                ...
                <div class="mt-2">
                    ...
                    <div
                        ...
                        <h1
                            class="text-xl font-semibold text-gray-800 transition duration-500 ease-in-out dark:text-white dark:group-hover:text-blue-200">
                            {{ strip_tags($post->title) }}</h1>

                        <p class="py-2 text-gray-700 dark:text-gray-400">
                            {{ strip_tags(Str::limit($post->body, 90))}}
                        </p>
                    </div>
                </div>
            </a>
    ...
    @foreach($posts as $post)
            <div class="p-6">
                ...
                <h1 class="mx-auto mb-8 text-2xl font-semibold leading-none tracking-tighter text-neutral-600 lg:text-3xl">
                    ({{ strip_tags($post->title) }}) </h1>
                <p class="mx-auto text-base leading-relaxed text-gray-500"> ({{strip_tags (Str::limit($post->body, 90))}})</p>
                <div class="mt-4">
                    <a href="#"
